...
export const Hero3D = ({ onOpenCV }) => {
  ...
  useEffect(() => {
    const updateTime = () => {
      try {
        ...
        setTimeWroclaw(new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }));
      }
    };
    updateTime();
  ...
  useEffect(() => {
    ...
    const getDims = () => {
      const w = Math.max(container.clientWidth || window.innerWidth || 390, 1);
      const h = Math.max(container.clientHeight || window.innerHeight || 600, 1);
      return { w, h };
    };
    ...
    try {
      ...
      renderer = new THREE.WebGLRenderer({ 
        antialias: !isMobileInit, // disable antialias on mobile to conserve GPU memory
        alpha: true,
        powerPreference: 'high-performance'
      });
      renderer.setSize(initW, initH);
      renderer.setPixelRatio(isMobileInit ? Math.min(window.devicePixelRatio || 1, 1.5) : Math.min(window.devicePixelRatio || 1, 2));
      renderer.domElement.style.touchAction = 'pan-y';
      renderer.domElement.style.pointerEvents = isMobileInit ? 'none' : 'auto';
      ...
      const animate = () => {
        ...
        satellites.forEach((sat) => {
          ...
          // Update line buffer in-place without garbage collection allocations
          const posAttr = sat.conduit.geometry.attributes.position;
          if (posAttr) {
      ...
      console.warn("Hero3D WebGL initialization fallback:", err);
    }
